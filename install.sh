...
install_node() {
    ...
    eval "$(fnm env)"
    fnm install --lts
    # fnm 不支持 lts-latest 别名，需要获取实际版本号后设置 default
    local lts_version
    lts_version=$(fnm list | grep -i lts-latest | awk '{print $2}' || true)
    if [ -n "$lts_version" ]; then
        fnm default "$lts_version"
        _ok "Node LTS ($lts_version) 已安装并设为默认"
    else
        # fallback: 用 fnm current 获取刚安装的版本
